...
function startGame() {
  if (balance < gameState.currentBet) {
    showNotification('Sensory account balance', 'error');
    return;
  }
...
function checkAndStartAutoPlay() {
  ...
  if (profit >= autoPlaySettings.stopProfit) {
    stopAutoPlay('Profit stop is reached!');
    return;
  }
  
  if (loss >= autoPlaySettings.stopLoss) {
    stopAutoPlay('Stop loss reached!');
    return;
  }
  
  if (gameState.gamesPlayed >= autoPlaySettings.gamesCount) {
    stopAutoPlay('Maximum number of games reached!');
    return;
  }
  
  if (balance < gameState.currentBet) {
    stopAutoPlay('Significant account balance!');
    return;
  }
