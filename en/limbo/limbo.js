function checkLoggedInUser() {
  ...
  if (!username || !storedBalance) {
    window.location.href = "/en/login/login.html";
    return;
  }
...
function updateStrategy() {
  ...
  switch (gameState.strategy) {
    case 'flat':
      strategyDescription.textContent = 'Flat: bet the same amount every time.';
      break;
    case 'martingale':
      strategyDescription.textContent = 'Martingale: Double bet reset after each loss after the victory.';
      break;
    case 'd-alembert':
      strategyDescription.textContent = "D'Alembert: If you increase the bet by one unit after the loss, decrease by one unit after victory.";
      break;
    case 'fibonacci':
      strategyDescription.textContent = 'Fibonacci: Follow the Fibonacci sequence for bets after losses and move two steps back after the victory.';
      break;
  }
...
function startGame() {
  ...
  if (balance < gameState.betAmount) {
    showMessage('Inadequate account balance!', 'lose');
    return;
  }
...
function playRound() {
  ...
  if (isWin) {
    ...
    showMessage(`You win ${profit.toFixed(2)}!`, 'win');
  } else {
    gameState.losses++;
    gameState.totalProfit -= gameState.betAmount;
    showMessage('Lost!', 'lose');
  }
  ...
  if (gameState.autoPlay) {
    ...
    if (gameState.autoPlayCount === 0 || gameState.currentAutoPlayCount < gameState.autoPlayCount) {
      if (balance >= gameState.betAmount) {
        ...
        playButton.classList.remove('hidden');
        showMessage('Auto Play stopped: inadequate account balance', 'lose');
      }
    } else {
      stopAutoPlay();
      gameState.isPlaying = false;
