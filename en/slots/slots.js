...
class SlotMachine {
    ...
    async spin() {
        ...
        if (this.freeSpins > 0) {
            this.freeSpins--;
            this.showMessage(`Gratis Spins: ${this.freeSpins}`, '#4a90e2');
        } else {
        ...
        if (result.totalWin > 0) {
            ...
            if (multiplier >= 50) {
                ...
                this.showMessage('HUGE WIN!', '#ff4e50');
                this.playWinAnimation('big');
            } else {
                this.showMessage(`Win: $${result.totalWin}!`);
            }
        }

        if (result.freeSpins > 0) {
            this.freeSpins += result.freeSpins;
            this.showMessage(`${result.freeSpins} Free spins won!`, '#4a90e2');
        }
