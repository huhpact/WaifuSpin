...
class SlotMachine {
    ...
    async spin() {
        ...
        if (this.freeSpins > 0) {
            this.freeSpins--;
            this.showMessage(`グラティススピン: ${this.freeSpins}`, '#4a90e2');
        } else {
        ...
        if (result.totalWin > 0) {
            ...
            if (multiplier >= 50) {
                this.showMessage('大勝利！', '#ffd700');
                this.playWinAnimation('mega');
            } else if (multiplier >= 20) {
                this.showMessage('大勝利！', '#ff4e50');
                this.playWinAnimation('big');
            } else {
                this.showMessage(`勝つ: $${result.totalWin}!`);
            }
        }

        if (result.freeSpins > 0) {
            this.freeSpins += result.freeSpins;
            this.showMessage(`${result.freeSpins} 無料のスピンが勝ちました！`, '#4a90e2');
        }
        ...
        if (this.isAutoSpinning && (this.balance >= this.bet || this.freeSpins > 0)) {
            ...
            this.isAutoSpinning = false;
            this.autoSpinButton.textContent = '自動';
        }
    }
    ...
    toggleAutoSpin() {
        ...
        this.autoSpinButton.textContent = this.isAutoSpinning ? '停止' : '自動';
