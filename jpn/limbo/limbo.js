...
function updateStrategy() {
  ...
  switch (gameState.strategy) {
    case 'flat':
      strategyDescription.textContent = 'フラット: 毎回同じ量を賭けます。';
      break;
    case 'martingale':
      strategyDescription.textContent = 'Martingale: ダブルベットは、勝利後の各損失の後にリセットされます。';
      break;
    case 'd-alembert':
      strategyDescription.textContent = "D'Alembert: 損失後に1ユニット単位増加した場合は、勝利後1ユニット減少します。";
      break;
    case 'fibonacci':
      strategyDescription.textContent = 'Fibonacci: 損失後の賭けのためにフィボナッチシーケンスに従い、勝利後に2歩後退します。';
      break;
  }
...
function startGame() {
  ...
  if (balance < gameState.betAmount) {
    showMessage('アカウントの残高が不十分です！', 'lose');
    return;
  }
  ...
  if (gameState.autoPlay) {
    ...
    if (gameState.autoPlayCount === 0 || gameState.currentAutoPlayCount < gameState.autoPlayCount) {
      if (balance >= gameState.betAmount) {
        ...
        playButton.classList.remove('hidden');
        showMessage('オートプレイの停止：アカウントの残高が不十分です', 'lose');
      }
    } else {
      stopAutoPlay();
      gameState.isPlaying = false;
