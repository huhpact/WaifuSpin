...
function startGame() {
  if (balance < gameState.currentBet) {
    showNotification('感覚アカウントのバランス', 'error');
    return;
  }
...
function checkAndStartAutoPlay() {
  ...
  if (profit >= autoPlaySettings.stopProfit) {
    stopAutoPlay('利益停止に達します！');
    return;
  }
  
  if (loss >= autoPlaySettings.stopLoss) {
    stopAutoPlay('停止損失に達しました！');
    return;
  }
  
  if (gameState.gamesPlayed >= autoPlaySettings.gamesCount) {
    stopAutoPlay('到達したゲームの最大数！');
    return;
  }
  
  if (balance < gameState.currentBet) {
    stopAutoPlay('重要なアカウント残高！');
    return;
  }
...
function stopAutoPlay(reason) {
  ...
  showNotification('オートプレイが停止しました： ' + reason, 'info');
}
...
function toggleAutoPlay() {
  ...
  autoPlayButton.textContent = gameState.autoPlayEnabled ? '自動を停止します' : '自動';
