class LuckyWheel {
  ...
  loadUserData() {
      ...
      document.getElementById('balance').textContent = `バランス: $${parseFloat(balance).toFixed(2)}`;
      document.getElementById('username').textContent = username;
  }

  getBalance() {
      const balanceText = document.getElementById('balance').textContent;
      return parseFloat(balanceText.replace('バランス: $', ''));
  }

  setBalance(amount) {
      const newBalance = amount.toFixed(2);
      document.getElementById('balance').textContent = `バランス: $${newBalance}`;
