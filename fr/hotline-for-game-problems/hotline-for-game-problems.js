...
function showQuestion() {
	...
	let html = `
			...
			<div class="quiz__navigation">
					${currentQuestion > 0 ? '<button onclick="previousQuestion()" class="secondary__button">Précédent</button>' : ''}
					<button onclick="nextQuestion()" class="primary__button">Suivant</button>
			</div>
	`;
...
function showResults() {
	...
	quizContent.innerHTML = `
			<div class="quiz__results">
					...
					<div class="quiz__navigation">
							<button onclick="closeQuiz()" class="primary__button">Fermer</button>
							<button onclick="startChat()" class="secondary__button">S'adresser à l'assistance</button>
					</div>
			</div>
	`;
}
