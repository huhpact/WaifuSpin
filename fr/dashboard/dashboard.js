let userData = {
    username: localStorage.getItem('loggedInUserUsername') || 'Invité',
...
document.querySelectorAll('.amount__buttons button').forEach(button => {
    button.addEventListener('click', () => {
        ...
        alert(`Déposé avec succès $${amount}! Vous serez redirigé vers Stripe.`);
    });
});
...
document.getElementById('withdrawBtn').addEventListener('click', () => {
    ...
    if (!wallet) {
        alert('Veuillez saisir une adresse de portefeuille valide');
        return;
    }
