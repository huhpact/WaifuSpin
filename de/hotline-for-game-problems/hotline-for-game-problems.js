...
function showQuestion() {
	...
	let html = `
			...
			<div class="quiz__navigation">
					${currentQuestion > 0 ? '<button onclick="previousQuestion()" class="secondary__button">Vorherige</button>' : ''}
					<button onclick="nextQuestion()" class="primary__button">Nächste</button>
			</div>
	`;
...
function showResults() {
	...
	quizContent.innerHTML = `
			<div class="quiz__results">
					...
					<div class="quiz__navigation">
							<button onclick="closeQuiz()" class="primary__button">Schließen</button>
							<button onclick="startChat()" class="secondary__button">Sich an den Support wenden</button>
					</div>
			</div>
	`;
}
