let userData = {
    username: localStorage.getItem('loggedInUserUsername') || 'Guest',
    balance: parseFloat(localStorage.getItem('loggedInUserBalance')) || "00.00"
};

const greetings = [
    `Willkommen zurück, ${userData.username}! Bereit zum Spinnen?`,
    `Hey ${userData.username}! Lass uns heute deinen Glückstag machen!`,
    `Schön dich zu sehen, ${userData.username}! Die Slots sind heute heiß!`
];
...
document.querySelectorAll('.amount__buttons button').forEach(button => {
    button.addEventListener('click', () => {
        ...
        alert(`Du hast erfolgreich $${amount} eingezahlt! Du wirst jetzt zur Bezahlung weitergeleitet.`);
    });
});
