...
function updateStrategy() {
  ...
  switch (gameState.strategy) {
    case 'flat':
      strategyDescription.textContent = 'Flach: Wetten Sie jedes Mal den gleichen Betrag.';
      break;
    case 'martingale':
      strategyDescription.textContent = 'Martingale: Doppelte Wette nach jedem Verlust, nach dem Sieg zurücksetzen.';
      break;
    case 'd-alembert':
      strategyDescription.textContent = "D'Alembert: Erhöhen Sie die Wette um eine Einheit nach dem Verlust, sinken um eine Einheit nach dem Sieg.";
      break;
    case 'fibonacci':
      strategyDescription.textContent = 'Fibonacci: Befolgen Sie die Fibonacci-Sequenz für Wetten nach Verlusten und bewegen Sie sich zwei Schritte nach dem Sieg zurück.';
      break;
  }
...
function startGame() {
  ...
  if (balance < gameState.betAmount) {
    showMessage('Unzureichender Kontostand!', 'lose');
    return;
  }
...
function playRound() {
  ...
  if (isWin) {
    ...
    showMessage(`Du gewinnst ${profit.toFixed(2)}!`, 'win');
  } else {
    gameState.losses++;
    gameState.totalProfit -= gameState.betAmount;
    showMessage('Verloren!', 'lose');
  }
  ...
  if (gameState.autoPlay) {
    ...
    if (gameState.autoPlayCount === 0 || gameState.currentAutoPlayCount < gameState.autoPlayCount) {
      if (balance >= gameState.betAmount) {
        ...
        playButton.classList.remove('hidden');
        showMessage('Auto Play gestoppt: Unzureichender Kontostand', 'lose');
      }
    } else {
      stopAutoPlay();
      gameState.isPlaying = false;
