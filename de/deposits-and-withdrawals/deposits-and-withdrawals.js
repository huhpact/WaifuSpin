...
function initPaymentMethods() {
  ...
  paymentMethods.forEach(method => {
    method.addEventListener('click', () => {
      ...
      console.log(`Ausgewählte Zahlungsmethode: ${paymentType}`);
    });
  });
}
    ...
    if (currentBox >= inputBoxes.length) {
      ...
      if (progressSteps.length >= 4) {
        setTimeout(() => {
          ...
          alert('Auszahlung erfolrgreich verarbeitet');
        }, 2000);
      }
    } else {
      alert('Bitte gebe den vollständigen 2FA-Code ein');
    }
  });
}
...
function initAnimations() {
  ...
  let currentStep = 2; // Paiement en cours
